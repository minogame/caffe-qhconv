...
__global__ void MinBackward(const int nthreads, const Dtype* top_diff,
    const int blob_idx, const int* mask, Dtype* bottom_diff, bool back_prop_all_error_) {
  CUDA_KERNEL_LOOP(index, nthreads) {
    Dtype gradient = 0;
    if (back_prop_all_error_ == false) {
      if (mask[index] == blob_idx) {
        gradient += top_diff[index];
      }
      bottom_diff[index] = gradient;
    ...
    if (propagate_down[i]) {
      ...
      case EltwiseParameter_EltwiseOp_MIN:
        ...
        MinBackward<Dtype>  // NOLINT_NEXT_LINE(whitespace/operators)
            <<<CAFFE_GET_BLOCKS(count), CAFFE_CUDA_NUM_THREADS>>>(
            count, top_diff, i, mask, bottom_diff, back_prop_all_error_);
        break;
      default:
        LOG(FATAL) << "Unknown elementwise operation.";
      }
