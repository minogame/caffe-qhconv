...
void loadXML(std::string xml, std::vector<Anno> &vAnnos) {
	fstream fs;
	...
	while (!fs.eof()) {
		fs>>tempAnno.ID;
		// std::cout<<tempAnno.ID<<std::endl;
		for (int i = 0; i < 4; ++i)
			fs>>tempAnno.pos[i];
		// std::cout<<tempAnno.pos[0]<<" "<<tempAnno.pos[1]<<" "<<tempAnno.pos[2]<<" "<<tempAnno.pos[3]<<std::endl;
...
int main(int argc, char** argv) {
	...
	db_image->Open("COCO_2014_train", db::NEW);
	db_anno->Open("COCO_2014_train_anno", db::NEW);
	scoped_ptr<db::Transaction> txn_image(db_image->NewTransaction());
	scoped_ptr<db::Transaction> txn_anno(db_anno->NewTransaction());

	// Storing to db
	std::string image_root_folder("/ceph/sun/DB/coco/images/");
	std::string anno_root_folder("/ceph/sun/DB/coco/annotations/");
	Datum image_datum;
	Datum anno_datum;
	...
	for (int line_id = 0; line_id < lines.size(); ++line_id) {
		...
		status = ReadImageToDatum(image_root_folder + lines[line_id].first + ".jpg",
